...
from BERT.Utils.utils import get_device
from BERT.Utils.load_datatsets import load_data
from BERT.Utils.train_evalute import train, evaluate_save
...
class Instructor(object):
    ...
    def run(self):
        ...
        if self.arguments.do_train:
            ...
            train_news, dev_news = news[index[0:int(len(index) * 0.8)]], news[index[int(len(index) * 0.8):]]
            train_category, dev_category = category[index[0:int(len(index) * 0.8)]], category[
                index[int(len(index) * 0.8):]]
            ...
            train(epoch_num=self.args.EPOCHS, n_gpu=n_gpu, model=model, train_dataloader=train_dataloader,
                  dev_dataloader=dev_dataloader,
                  optimizer=optimizer, scheduler=scheduler, criterion=criterion,
                  gradient_accumulation_steps=self.arguments.gradient_accumulation_steps, device=device,
                  label_list=self.arguments.label_list,
                  output_model_file=output_model_file, output_config_file=output_config_file,
                  log_dir=self.arguments.log_dir, print_step=self.arguments.print_step,
                  early_stop=self.arguments.early_stop)
